...
public class DaoJDBCUtil {
     ...
     * <p>
     * <b>TestFixtrue가 반드시 할당되어 있어야합니다.
     */
     ...
     * <p>
     * <b>TestFixtrue가 반드시 할당되어 있어야합니다.</b>
     * 
    ...
    public void deleteRefEntity(Class<?> entityClass) {
        ...
        Object primaryKey = Arrays.stream(key.getClass().getDeclaredFields())
                ...
                .orElseThrow(() -> new IllegalStateException(
                        "PrimaryKey 어노테이션이 존재하지 않습니다. " + key.getClass().getSimpleName() + " 엔티티를 확인해 주세요."));
        this.daoMethodInvoke(key, "delete", primaryKey);
    }

    // 참조 테이블에 엔티티를 추가하는 함수
    public void addRefEntity(Object refEntity) {
        Class<?> refEntityClass = refEntity.getClass();
        Object key = this.testFixture.refEntityAndDao.keySet().stream()
                .filter(refentity -> refentity.getClass().getSimpleName().equals(refEntityClass.getSimpleName()))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("addRefEntity()의 매개변수로 잘못된 클래스를 전달받았습니다."));

        this.daoMethodInvoke(key, "add", refEntity);
    }
